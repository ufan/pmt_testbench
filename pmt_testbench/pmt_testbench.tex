...
By recording the reponse of PMT at the same light intensity, the relative gain of all tubes can be measured.
This method is more efficient than the absolute measurement of gain using single photon electron spectrum.
However, two additional corrections are needed to get an accurate result as follows: 
\begin{equation}
 G_{relative}=\frac{A_{mean}}{K_{runid} T_{chid}}
\end{equation} 
where $A_{mean}$ is the mean value of the raw ADC spectrum at the specified light intensity, $K_runid$ is the light source  
$K_{runid}$ can be calculated using the two reference PMTs described in Sec.\ref{sec:description}.
$T_{chid}$ is a fixed parameter, which has been calibrated in Sec.\ref{sec:fibre_bundle}.

\subsection{Testing Result of the Ratio between Dynode8 and Dynode5}
\label{sec:psd_dy58}
