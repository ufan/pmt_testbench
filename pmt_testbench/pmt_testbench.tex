...
\end{equation} 
where $A_{mean}$ is the mean value of the raw ADC spectrum at the specified light intensity,
$K_{runid}$ is the light source fluctuation between different test runs and $T_{chid}$ is the light transmition difference among testing channels.
$K_{runid}$ can be calculated at each test run using the two reference PMTs described in Sec.\ref{sec:description}.
...
\subsection{Cathode Uniformity of R4443 Mod2}
