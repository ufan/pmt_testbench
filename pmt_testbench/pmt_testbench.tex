...
As a first step,all used concrete device classes will be registered in the singlton class \textit{PTDeviceManager}.
...
\textit{PTVTest} encapsulates the real testing operations,which corresponds to a specific set of testing conditions or device parameters.
Usually,a \textit{PTVProgram} is consist of multiple \textit{PTVTest}s.
...
Besides real testing operations,additional actions may be added in the \textit{PreTest} method and \textit{PostTest} method of \textit{PTVProgram}.
For example,PMT warming may be performed in \textit{PreTest} and analysis code may be added in \textit{PostTest}.
Two concrete \textit{PTVProgram}s are implemented for DAMPE-PSD PMT test.
