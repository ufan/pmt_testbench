...
A high-power blue LED(\SI{3}{\watt}, \SIrange{465}{485}{\nano\meter}) from Z-light~\cite{zlight} is apdopted as the light source for the test bench, which has been used before in the monitoring and calibration system of Neutron Wall Detector at IMP~\cite{yuyuhong_led}. 
The LED is fixed on a special designed base using thermally conductive silicone rubber and then coupled to a \SI{5}{\centi\meter} diameter integrating sphere directly, making the whole sphere as its heat sink.
The integrating sphere is coated interiorly with highly reflective material($\approx$\SI{98}{\percent} at \SI{400}{\nano\meter}), thus converting the LED into a perfect uniform light source(\ref{fig:uniformity_integratingsphere}).
This is important
